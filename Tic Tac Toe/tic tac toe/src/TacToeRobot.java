...
public class TacToeRobot implements ActionListener {
    ...
    {   
        if (e.getSource() == ChoiceX) {
            PlayerX = true;
            robot = false;
            currentPlayerX = true; // Set the initial player's turn to "X"
        } else if (e.getSource() == Choice0) {
            PlayerX = false;
            robot = true;
            currentPlayerX = true; // Set the initial player's turn to "X"
        }
        if(e.getSource() == ChoiceX || e.getSource() == Choice0)
        {
        ...
        for(int i = 0 ; i < 9; i++)
        {
            if(e.getSource() == buttons[i])
            {
                ...
                else if (Player0 && buttons[i].getText().equals("")) // or (robot && ...)
                {
                     {
                        buttons[i].setForeground(new Color(255, 0, 0));
                        buttons[i].setText("X");
